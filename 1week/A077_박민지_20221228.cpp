/*
Source: https://www.acmicpc.net/problem/2822
...
int main(){
	...
	for(int i=0; i<8; i++){
		cin >> get_score;
		score.push_back(get_score);
	}
	score_temp = score;

	sort(score_temp.rbegin(), score_temp.rend());
	for(int i=0; i<5; i++){
		result += score_temp[i];
		auto it = find(score.begin(), score.end(), score_temp[i]);
		num.push_back(it-score.begin()+1);
	}
